...
public final class HttpResponseHelper {
     ...
     * 校验并返回提供商配置
     */
    public static AIModelProperties.ProviderConfig checkAndReturnProvider(ModelRef model, String label) {
     ...
     * 校验提供商 API 密钥
     */
    public static void checkApiKey(AIModelProperties.ProviderConfig provider, String label) {
     ...
     * 校验并返回模型名称
     */
    public static String checkAndReturnModel(ModelRef model, String label) {
        if (model == null || model.candidate() == null || model.candidate().getModel() == null) {
            throw new IllegalStateException(label + " 模型名称缺失");
        }
        return model.candidate().getModel();
    }
}
